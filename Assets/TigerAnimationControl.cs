...
public class TigerAnimationControl : MonsterBase
{
	...
	void Update () {
        ...
        if (Input.GetKeyDown(KeyCode.Space)) tryChangeBool("Attack", true);
        if (Input.GetKeyDown(KeyCode.Z))
        {
            tryChangeBool("Sound", true);
        }

        if (currentBaseState.nameHash == AttState)
        {
            tryChangeBool("Attack", false);
        }
        if (currentBaseState.nameHash == SoundState)
        {
            tryChangeBool("Sound", false);
        }
